...
$(function(){
	...
	$('#main-heading>li').each(function(i){
		$('#nav-main').append(createMenuItem($(this),(i+1),1,observer));
	});
	setTimeout(function(){
		initializing=false;	
	},1000);
...
function createMenuItem(li,id,level,observer){
	id=id+'-';
	...
	if(li.children('.heading').length==1){
		...
		li.children('.heading').first().children('li').each(function(i){
			ul.append(createMenuItem($(this),id+(i+1),level+1,observer));
		});
		item.append(ul);
	}
